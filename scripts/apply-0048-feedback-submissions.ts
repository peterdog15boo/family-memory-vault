/**
 * Apply drizzle/0048_feedback_submissions.sql (statement-by-statement).
 */
...
async function main() {
  ...
  const sql = neon(url);
  const path = "drizzle/0048_feedback_submissions.sql";
  const contents = readFileSync(path, "utf8");
  const hash = createHash("sha256").update(contents).digest("hex");

  // Split on statement-breakpoint; keep DO $$ ... $$ blocks intact.

...
/**
 * POST /api/legacy/trust/signed-scan/upload-url
 * Presign PUT under private-legacy-trusts-temp/{userId}/ only.
 */
export async function POST(request: Request) {
  ...
  if (!isTrustSignedScanContentType(parsed.data.contentType)) {
    return apiError("Signed trust scans must be PDF, JPEG, or PNG.", {
      status: 400,
      code: "validation",
    });
  }
  ...
  try {
    ...
    if (!draft || draft.status === "archived") {
      return apiError("Trust draft not found", {
        status: 404,
        code: "not_found",
      });
    }
    ...
    const upload = await createTrustSignedScanUploadUrl({
      userId,

...
/** POST /api/legacy/will/accept-disclaimer */
export async function POST(request: Request) {
  ...
  const limited = enforceRateLimit(
    `will-disclaimer:${authResult.userId}`,
  ...
  if (!parsed.success) {
    return apiError("Please accept the Will Planner disclaimer to continue.", {
      status: 400,
      code: "validation",
    });
  }
  ...
  try {
    const row = await recordWillDisclaimerAcceptance({
